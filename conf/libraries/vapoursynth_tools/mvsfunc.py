...
dither=None, useZ=None, prefer_props=None, ampo=None, ampn=None, dyn=None, staticnoise=None):
    ...
    if sColorFamily == vs.COMPAT:
        raise ValueError(funcName +
                         ': color family *COMPAT* is not supported!')
    ...
    if dSType == vs.INTEGER and (dbitPS < 1 or dbitPS > 16):
        raise ValueError(
            funcName +
            ': {0}-bit integer output is not supported!'.format(dbitPS))
    if dSType == vs.FLOAT and (dbitPS != 16 and dbitPS != 32):
        raise ValueError(funcName + ': {0}-bit float output is not supported!'.
                         format(dbitPS))
    ...
    if lowDepth:
        if dither == "none" or dither == 1:
            clip = _quantization_conversion(clip, sbitPS, depth, vs.INTEGER,
                                            fulls, fulld, False, False, 8, 0,
                                            funcName)
            clip = _quantization_conversion(clip, depth, 8, vs.INTEGER, fulld,
                                            fulld, False, False, 8, 0,
                                            funcName)
            return clip
        else:
            full = fulld
            clip = _quantization_conversion(clip, sbitPS, depth, vs.INTEGER,
                                            fulls, full, False, False, 16, 1,
                                            funcName)
            sSType = vs.INTEGER
            sbitPS = 16
            fulls = False
    ...
    if dSType == vs.INTEGER and (dbitPS == 13 or dbitPS == 15):
        useZ = True
    if (sSType == vs.FLOAT and sbitPS < 32) or (dSType == vs.FLOAT
                                                and dbitPS < 32):
        useZ = True
    ...
    # Dithering type
    if ampn is not None and not isinstance(ampn, float) and not isinstance(
            ampn, int):
        raise TypeError(funcName + ': \"ampn\" must be an int or a float!')

    if dither is None:
        if dbitPS == 32 or (dbitPS >= sbitPS and fulld == fulls
                            and fulld == False):
    ...
    if dSType == sSType and dbitPS == sbitPS and (
            sSType == vs.FLOAT or
        (fulld == fulls and not prefer_props_range)) and not lowDepth:
        return clip
    ...
    # Low-depth support
    if lowDepth:
        clip = _quantization_conversion(clip, depth, 8, vs.INTEGER, full, full,
                                        False, False, 8, 0, funcName)
...
compat=None):
    ...
    if full is None:
        ...
        if (sIsGRAY or sIsYUV
                or sIsYCOCG) and (matrix == "RGB" or matrix == "YCgCo"
                                  or matrix == "OPP"):
            fulls = True
        else:
            fulls = False if sIsYUV or sIsGRAY else True
    ...
    if sIsRGB:
        ...
        clip = Depth(clip, dbitPS, dSType, fulls, fulld, dither, useZ,
                     prefer_props, ampo, ampn, dyn, staticnoise)
    elif sIsGRAY:
        # Apply depth conversion for output clip
        clip = Depth(clip, dbitPS, dSType, fulls, fulld, dither, useZ,
                     prefer_props, ampo, ampn, dyn, staticnoise)
        ...
        # Apply depth conversion for processed clip
            ...
            clip = Depth(clip, pbitPS, pSType, fulls, fulls, dither, useZ,
                         prefer_props, ampo, ampn, dyn, staticnoise)
        ...
        if matrix == "OPP":
            ...
            clip = core.fmtc.matrix(
                clip, mat=matrix, fulls=fulls, fulld=fulld, col_fam=vs.RGB)
        # Apply depth conversion for output clip
        clip = Depth(clip, dbitPS, dSType, fulld, fulld, dither, useZ,
                     prefer_props, ampo, ampn, dyn, staticnoise)
    ...
    if sIsYUV or sIsYCOCG:
        ...
        if matrix == "OPP":
            clip = core.fmtc.matrix(
                ...
                fulld=fulld,
                coef=[
                    1 / 3, 1 / 3, 1 / 3, 0, 1 / 2, 0, -1 / 2, 0, 1 / 4, -1 / 2,
                    1 / 4, 0
                ],
                col_fam=vs.YUV)
...
block_size2=None, block_step2=None, group_size2=None, bm_range2=None, bm_step2=None, ps_num2=None, ps_range2=None, ps_step2=None, th_mse2=None):
    ...
    if sigma is None:
        ...
        if isinstance(sigma, int):
            ...
            raise TypeError(funcName +
                            ': sigma must be a float[] or an int[]!')
    if sIsGRAY:
        sigma = [sigma[0], 0, 0]
    ...
    if radius1 is None:
        ...
        raise ValueError(funcName +
                         ': valid range of \"radius1\" is [0, +inf)!')
    if radius2 is None:
        radius2 = radius1
        ...
        raise ValueError(funcName +
                         ': valid range of \"radius2\" is [0, +inf)!')
    ...
    if refine is None:
        ...
        raise ValueError(funcName +
                         ': valid range of \"refine\" is [0, +inf)!')
    ...
    if output is None:
        ...
        raise ValueError(funcName +
                         ': valid values of \"output\" are 0, 1 and 2!')
    ...
    if sIsGRAY:
        ...
        clip = Depth(clip, pbitPS, pSType, fulls, True, dither, useZ,
                     prefer_props, ampo, ampn, dyn, staticnoise)
        if pre is not None:
            pre = Depth(pre, pbitPS, pSType, fulls, True, dither, useZ,
                        prefer_props, ampo, ampn, dyn, staticnoise)
        if ref is not None:
            ref = Depth(ref, pbitPS, pSType, fulls, True, dither, useZ,
                        prefer_props, ampo, ampn, dyn, staticnoise)
    else:
    ...
    for i in range(0, refine):
        if skip:
            ...
            # Shuffle Y plane back if not processed
            if not onlyY and sigma[0] <= 0:
                flt = core.std.ShufflePlanes([clip, flt, flt], [0, 1, 2],
                                             vs.YUV)

    # Convert to output format
    if sIsGRAY:
        clip = Depth(flt, dbitPS, dSType, True, fulld, dither, useZ,
                     prefer_props, ampo, ampn, dyn, staticnoise)
    else:
        # Shuffle back to YUV if not all planes are processed
        if onlyY:
            clip = core.std.ShufflePlanes([flt, srcOPP, srcOPP], [0, 1, 2],
                                          vs.YUV)
        ...
        if output <= 0 and not sIsRGB:
            ...
            clip = Depth(clip, dbitPS, dSType, True, fulld, dither, useZ,
                         prefer_props, ampo, ampn, dyn, staticnoise)
...
def VFRSplice(clips, tcfile=None, v2=None, precision=None):
    ...
    if isinstance(clips, vs.VideoNode):
        ...
        for clip in clips:
            if not isinstance(clip, vs.VideoNode):
                raise TypeError(funcName +
                                ': each element in \"clips\" must be a clip!')
            if clip.fps_num == 0 or clip.fps_den == 0:
                raise ValueError(funcName +
                                 ': each clip in \"clips\" must be CFR!')
    else:
        raise TypeError(funcName +
                        ': \"clips\" must be a clip or a list of clips!')
    ...
    if tcfile is None:
        ...
        index = 0
        for clip in clips:
            if index > 0 and clip.fps_num == tc_list[index -
                                                     1][2] and clip.fps_den == tc_list[index
                                                                                       -
                                                                                       1][3]:
                tc_list[index - 1] = (tc_list[index - 1][0],
                                      cur_frame + clip.num_frames - 1,
                                      clip.fps_num, clip.fps_den)
            else:
                tc_list.append((cur_frame, cur_frame + clip.num_frames - 1,
                                clip.fps_num, clip.fps_den))
        ...
        if v2:  # timecode v2
            ...
            olines = [
                '# timecode format v1\n', 'Assume {}\n'.format(
                    frac2str(tc_list[0][2], tc_list[0][3], precision))
            ]
            for tc in tc_list:
                olines.append('{},{},{}\n'.format(tc[0], tc[
                    1], frac2str(tc[2], tc[3], precision)))
        try:
...
def PlaneStatistics(clip,
    ...
    if plane is None:
        ...
        raise ValueError(
            funcName + ': valid range of \"plane\" is [0, sNumPlanes)!'.format(
                sNumPlanes=sNumPlanes))
    ...
    if mad:
        ...
        ADclip = core.std.FrameEval(clipPlane,
                                    functools.partial(
                                        _PlaneADFrame, clip=clipPlane), clip)
        ...
        clip = core.std.ModifyFrame(
            clip, [clip, ADclip], selector=_PlaneMADTransfer)
    ...
    if var or std:
        ...
        SDclip = core.std.FrameEval(floatBlk,
                                    functools.partial(
                                        _PlaneSDFrame, clip=clipPlane), clip)
        SDclip = PlaneAverage(SDclip, 0, "PlaneVar")

        def _PlaneVarSTDTransfer(n, f):
            fout = f[0].copy()
            if var:
                fout.props.PlaneVar = f[1].props.PlaneVar / (
                    valueRange * valueRange)
            if std:
                fout.props.PlaneSTD = math.sqrt(
                    f[1].props.PlaneVar) / valueRange
            return fout

        clip = core.std.ModifyFrame(
            clip, [clip, SDclip], selector=_PlaneVarSTDTransfer)
    ...
    if rms:
        ...
        clip = core.std.ModifyFrame(
            clip, [clip, squareClip], selector=_PlaneRMSTransfer)
...
def PlaneCompare(clip1,
    ...
    if sFormat.id != clip2.format.id:
        raise ValueError(
            funcName + ': \"clip1\" and \"clip2\" must be of the same format!')
    if clip1.width != clip2.width or clip1.height != clip2.height:
        raise ValueError(
            funcName +
            ': \"clip1\" and \"clip2\" must be of the same width and height!')
    ...
    if mae:
        ...
        clip1 = core.std.ModifyFrame(
            clip1, [clip1, ADclip], selector=_PlaneMAETransfer)
    ...
    if rmse or psnr:
        ...
        def _PlaneRMSEnPSNRTransfer(n, f):
            fout = f[0].copy()
            if rmse:
                fout.props.PlaneRMSE = math.sqrt(
                    f[1].props.PlaneMSE) / valueRange
            if psnr:
                fout.props.PlanePSNR = min(
                    10 * math.log(
                        valueRange * valueRange / f[1].props.PlaneMSE, 10),
                    99.99999999999999) if f[1].props.PlaneMSE > 0 else 100.0
            return fout

        clip1 = core.std.ModifyFrame(
            clip1, [clip1, SDclip], selector=_PlaneRMSEnPSNRTransfer)
    ...
    if cov or corr:
        ...
        CoDclip = core.std.FrameEval(floatBlk,
                                     functools.partial(
                                         _PlaneCoDFrame,
                                         clip1=clip1Plane,
                                         clip2=clip2Plane),
        ...
        if corr:
            ...
            SDclip1 = core.std.FrameEval(floatBlk,
                                         functools.partial(
                                             _PlaneSDFrame, clip=clip1Plane),
                                         clip1Mean)
            SDclip2 = core.std.FrameEval(floatBlk,
                                         functools.partial(
                                             _PlaneSDFrame, clip=clip2Plane),
                                         clip2Mean)
        ...
        def _PlaneCovTransfer(n, f):
            fout = f[0].copy()
            if cov:
                fout.props.PlaneCov = f[1].props.PlaneCov / (
                    valueRange * valueRange)
            if corr:
                std1std2 = math.sqrt(f[2].props.PlaneVar * f[3].props.PlaneVar)
                fout.props.PlaneCorr = f[
                    1].props.PlaneCov / std1std2 if std1std2 > 0 else float(
                        "inf")
            return fout
...
def ShowAverage(clip, alignment=None):
    ...
    valueRange = (1 << sbitPS) - 1 if sSType == vs.INTEGER else 1
    offset = [0, -0.5, -0.5
              ] if sSType == vs.FLOAT and (sIsYUV or sIsYCOCG) else [0, 0, 0]
...
def FilterIf(src, flt, prop_name, props=None):
    ...
    if sFormat.id != flt.format.id:
        raise ValueError(funcName +
                         ': \"src\" and \"flt\" must be of the same format!')
    if src.width != flt.width or src.height != flt.height:
        raise ValueError(
            funcName +
            ': \"src\" and \"flt\" must be of the same width and height!')

    if prop_name is None or not isinstance(prop_name, str):
        raise TypeError(funcName +
                        ': \"prop_name\" must be specified and must be a str!')
    else:
...
def LimitFilter(flt,
    ...
    if sFormat.id != src.format.id:
        raise ValueError(funcName +
                         ': \"flt\" and \"src\" must be of the same format!')
    if flt.width != src.width or flt.height != src.height:
        raise ValueError(
            funcName +
            ': \"flt\" and \"src\" must be of the same width and height!')

    if ref is not None:
        if sFormat.id != ref.format.id:
            raise ValueError(
                funcName + ': \"flt\" and \"ref\" must be of the same format!')
        if flt.width != ref.width or flt.height != ref.height:
            raise ValueError(
                funcName +
                ': \"flt\" and \"ref\" must be of the same width and height!')
    ...
    if brighten_thr is None:
        ...
        if brighten_thr < 0:
            raise ValueError(funcName +
                             ':valid range of \"brighten_thr\" is [0, +inf)')
    else:
        raise TypeError(funcName +
                        ': \"brighten_thr\" must be an int or a float!')
    ...
    if planes is None:
        ...
        if planes < 0 or planes >= VSMaxPlaneNum:
            raise ValueError(
                funcName +
                ': valid range of \"planes\" is [0, {VSMaxPlaneNum})!'.format(
        ...
        for p in planes:
            if not isinstance(p, int):
                raise TypeError(funcName +
                                ': \"planes\" must be a (list of) int!')
            elif p < 0 or p >= VSMaxPlaneNum:
                raise ValueError(
                    funcName +
                    ': valid range of \"planes\" is [0, {VSMaxPlaneNum})!'.
                    format(VSMaxPlaneNum=VSMaxPlaneNum))
            process[p] = 1
    else:
    ...
    if force_expr:  # implementation with std.Expr
        valueRange = (1 << sbitPS) - 1 if sSType == vs.INTEGER else 1
        limitExprY = _limit_filter_expr(ref is not None, thr, elast,
                                        brighten_thr, valueRange)
        limitExprC = _limit_filter_expr(ref is not None, thrc, elast, thrc,
                                        valueRange)
        expr = []
    ...
    if hpow > 0:
        ...
        for i in range(hpow):
            clip = core.std.Interleave(
                [clip, clip]).std.DoubleWeave(True).std.SelectEvery(2, 0)
        clip = core.std.Transpose(clip)
...
def CheckMatrix(clip, matrices=None, full=None, lower=None, upper=None):
    ...
    if not (sIsYUV or sIsYCOCG):
        raise ValueError(funcName +
                         ': only YUV or YCoCg color family is allowed!')
    ...
    def _FrameProps(n, f):
        ...
        fout.props.__setattr__(
            props[3],
            (f.props.__getattr__(props[0]) + f.props.__getattr__(props[1]) +
             f.props.__getattr__(props[2])) / 3)
        return fout

    rgb_clips = []
    for matrix in matrices:
        rgb_clip = ToRGB(clip, matrix=matrix)
        rgb_clip = rgb_clip.std.Expr('x {lower} < 1 x - x {upper} > x 0 ? ?'.
        ...
        rgb_clip = core.std.ModifyFrame(
            rgb_clip, rgb_clip, selector=_FrameProps)
    ...
    for item in expr_list:
        if op1.count(item) > 0:
            try:
                ...
                raise ValueError(funcName +
                                 ': Invalid expression, require operands.')
            if item == 'dup':
...
def SetColorSpace(clip,
    ...
    if ChromaLocation is None:
        ...
        if ChromaLocation is False:
            clip = core.std.SetFrameProp(
                clip, prop='_ChromaLocation', delete=True)
    elif isinstance(ChromaLocation, int):
        if ChromaLocation >= 0 and ChromaLocation <= 5:
            clip = core.std.SetFrameProp(
                clip, prop='_ChromaLocation', intval=ChromaLocation)
        else:
            raise ValueError(funcName +
                             ': valid range of \"ChromaLocation\" is [0, 5]!')
    else:
        raise TypeError(funcName +
                        ': \"ChromaLocation\" must be an int or a bool!')
    ...
    if ColorRange is None:
        ...
        if ColorRange >= 0 and ColorRange <= 1:
            clip = core.std.SetFrameProp(
                clip, prop='_ColorRange', intval=ColorRange)
        else:
            raise ValueError(funcName +
                             ': valid range of \"ColorRange\" is [0, 1]!')
    else:
        raise TypeError(funcName +
                        ': \"ColorRange\" must be an int or a bool!')
...
def CheckVersion(version, less=False, equal=True, greater=True):
    ...
    if not less and MvsFuncVersion < version:
        raise ImportWarning(
            'mvsfunc version(={0}) is less than the version(={1}) specified!'.
            format(MvsFuncVersion, version))
    if not equal and MvsFuncVersion == version:
        raise ImportWarning(
            'mvsfunc version(={0}) is equal to the version(={1}) specified!'.
            format(MvsFuncVersion, version))
    if not greater and MvsFuncVersion > version:
        raise ImportWarning(
            'mvsfunc version(={0}) is greater than the version(={1}) specified!'.
            format(MvsFuncVersion, version))
...
def zDepth(clip,
    ...
    format = core.register_format(sFormat.color_family, sample, depth,
                                  sFormat.subsampling_w, sFormat.subsampling_h)
...
def PlaneAverage(clip, plane=None, prop=None):
    ...
    if core.std.get_functions().__contains__('PlaneAverage'):
        ...
        raise AttributeError(funcName +
                             ': Available plane average function not found!')
...
def GrayScale(clip, matrix=None):
    ...
    if sIsGRAY:
        ...
        clip = core.std.ShufflePlanes([clip, blank, blank], [0, 1, 2],
                                      sColorFamily)
...
################################################################################################################################
def _quantization_parameters(sample=None,
                             depth=None,
                             full=None,
                             chroma=None,
                             funcName='_quantization_parameters'):
    qp = {}
    ...
    if sample == vs.INTEGER:
        if chroma:
            ...
            qp['ceil'] = (
                1 << depth
            ) - 1 if full else 240 << lShift if lShift >= 0 else 240 >> rShift
            ...
            qp['ceil'] = (
                1 << depth
            ) - 1 if full else 235 << lShift if lShift >= 0 else 235 >> rShift
            qp['range'] = qp['ceil'] - qp['floor']
...
clamp=None, dbitPS=None, mode=None, funcName='_quantization_conversion'):
    ...
    dFormat = core.register_format(sFormat.color_family, dSType, dbitPS,
                                   sFormat.subsampling_w,
                                   sFormat.subsampling_h)
    ...
    def gen_expr(chroma, mode):
        ...
        offset = dQP['neutral' if chroma else
                     'floor'] - sQP['neutral' if chroma else 'floor'] * gain
        ...
        if gain != 1 or offset != 0 or clamp:
            ...
            if clamp:
                if dQP['floor'] * scale > exprLower:
                    expr = expr + " {} max ".format(dQP['floor'] * scale)
                if dQP['ceil'] * scale < exprUpper:
                    expr = expr + " {} min ".format(dQP['ceil'] * scale)
        else:
            expr = ""
...
################################################################################################################################
def _check_arg_prop(arg,
                    default=None,
                    defaultTrue=None,
                    argName='arg',
                    funcName='_check_arg_prop'):
    ...
    if arg is None:
        ...
        if arg:
            if not arg.isidentifier():
                raise ValueError(
                    funcName +
                    ': {argName}=\"{arg}\" is not a valid identifier!'.format(
                        argName=argName, arg=arg))
        else:
            arg = False
    else:
        raise TypeError(funcName + ': \"{argName}\" must be a str or a bool!'.
...
def _operator2(clip1, clip2, mode, neutral, funcName):
    ...
    for i in range(sNumPlanes):
        if funcName == 'Min':
            if mode[i] >= 2:
                expr.append("y {neutral} - abs x {neutral} - abs < y x ?".
    ...
    for i in range(sNumPlanes):
        if process[i]:
            if funcName == 'MinFilter':
                ...
                raise ValueError(
                    '_min_max_filter: Unknown \"funcName\" specified!')
        else:
            expr.append("")
...
def _limit_filter_expr(defref, thr, elast, largen_thr, value_range):
    ...
    if thr <= 0 and largen_thr <= 0:
        ...
        if thr <= 0:
            ...
            limitExpr = " {dif_abs} {thr} <= {flt} {src} ? ".format(
                dif_abs=dif_abs, thr=thr, flt=flt, src=src)
        else:
            thr_1 = thr
            thr_2 = thr * elast
        ...
        if largen_thr != thr:
            if largen_thr <= 0:
                ...
                limitExprLargen = " {dif_abs} {thr} <= {flt} {src} ? ".format(
                    dif_abs=dif_abs, thr=largen_thr, flt=flt, src=src)
            else:
                thr_1 = largen_thr
                ...
                limitExprLargen = limitExprLargen.format(
                    dif=dif,
                    dif_abs=dif_abs,
                    thr_1=thr_1,
                    thr_2=thr_2,
