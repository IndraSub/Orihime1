...
def LDMerge(flt_h,
    ...
    hmap = core.std.Convolution(
        src, matrix=convknl_h, saturate=False, planes=planes, divisor=conv_div)
    vmap = core.std.Convolution(
        src, matrix=convknl_v, saturate=False, planes=planes, divisor=conv_div)
    ...
    ldexpr = '{peak} 1 x 0.0001 + y 0.0001 + / {power} pow + /'.format(
        peak=(1 << bits) - 1, power=power
    ) if calc_mode == 0 else 'y 0.0001 + x 0.0001 + dup * y 0.0001 + dup * + 2 * sqrt / {peak} *'.format(
        peak=(1 << bits) - 1)
    ldmap = core.std.Expr([hmap, vmap],
                          [(ldexpr if i in planes else '')
...
def Compare(src, flt, power=1.5, chroma=False, mode=2):
    ...
    if src.format.id != flt.format.id:
        raise ValueError(funcName +
                         ': \"flt\" must be of the same format as \"src\"!')
    if src.width != flt.width or src.height != flt.height:
        raise ValueError(funcName +
                         ': \"flt\" must be of the same size as \"src\"!')
    if mode not in [1, 2]:
    ...
    expr[2] = 'y x - {scale} * {neutral} +'.format(
        scale=32768 / (65536**(1 / power) - 1), neutral=32768)
    ...
    if bits < 16:
        ...
        diff = core.std.Expr(
            [src, flt], [expr[mode]]
            if chroma else [expr[mode], '{neutral}'.format(neutral=32768)])
        diff = core.fmtc.bitdepth(
            diff, bits=bits, dmode=1, fulls=True, fulld=True)
    else:
        diff = core.std.Expr(
            [src, flt], [expr[mode]]
            if chroma else [expr[mode], '{neutral}'.format(neutral=32768)])
...
def ExInpand(input, mrad=0, mode='rectangle', planes=None):
    ...
    if not isinstance(mode, list):
        raise TypeError(
            funcName +
            ': \"mode\" must be an int, a string, a list of ints, a list of strings or a list of mixing ints and strings!'
        )
    ...
    def ExInpand_process(input, mode=None, planes=None, mrad=None):
        ...
        if isinstance(mode, str):
            ...
            if mode not in ['rectangle', 'losange', 'ellipse']:
                raise ValueError(
                    funcName +
                    ': \"mode\" must be an int in [0, 2] or a specific string in [\"rectangle\", \"losange\", \"ellipse\"]!'
                )
        else:
            raise TypeError(
                funcName +
        ...
        if sw * sh < 0:
            raise TypeError(
                funcName +
                ': \"mrad\" at a time must be both positive or negative!')

        if sw > 0 or sh > 0:
            return haf.mt_expand_multi(
                input, mode=mode, planes=planes, sw=sw, sh=sh)
        else:
            return haf.mt_inpand_multi(
                input, mode=mode, planes=planes, sw=-sw, sh=-sh)
        ...
        for i in range(len(mrad)):
            if isinstance(mrad[i], list):
                if len(mrad[i]) != 1 and len(mrad[i]) != 2:
                    raise TypeError(
                        funcName +
                        ': \"mrad\" must be an int, a list of ints or a list of a list of two ints!'
                    )
                for n in mrad[i]:
            ...
            clip = ExInpand_process(
                input, mode=mode[i], planes=planes, mrad=mrad[i])
    else:
        raise TypeError(
            funcName +
            ': \"mrad\" must be an int, a list of ints or a list of a list of two ints!'
    ...
    if isinstance(msmooth, list):
        for m in msmooth:
            if not isinstance(m, int):
                raise TypeError(
                    funcName +
                    ': \"msmooth\" must be an int or a list of ints!')
            else:
                clip = InDeflate_process(input, planes=planes, radius=m)
    else:
        raise TypeError(funcName +
                        ': \"msmooth\" must be an int or a list of ints!')
...
def MultiRemoveGrain(input, mode=0, loop=1):
    ...
    if isinstance(mode, list):
        ...
        raise TypeError(
            funcName +
            ': \"mode\" must be an int, a list of ints or a list of a list of ints!'
        )
...
def GradFun3(src,
    ...
    if src.format.color_family not in [vs.YUV, vs.GRAY, vs.YCOCG]:
        raise TypeError(funcName +
                        ': \"src\" must be YUV, GRAY or YCOCG color family!')
    ...
    if radius is None:
        radius = (16 if src.width > 1024 or src.height > 576 else
                  12) if (smode == 1 or smode == 2) else 9
    ...
    if thr_det is None:
        ...
        if thr_det <= 0.0:
            raise ValueError(funcName +
                             '" \"thr_det\" must be strictly positive!')
    else:
    ...
    if radiusc is None:
        ...
        if radiusc <= 0:
            raise ValueError(funcName +
                             '\"radiusc\" must be strictly positive.')
    else:
    ...
    if elastc is None:
        ...
        if elastc < 1:
            raise ValueError(funcName +
                             ':valid range of \"elastc\" is [1, +inf)!')
    else:
    ...
    if ref.format.color_family not in [vs.YUV, vs.GRAY, vs.YCOCG]:
        raise TypeError(funcName +
                        ': \"ref\" must be YUV, GRAY or YCOCG color family!')
    if src.width != ref.width or src.height != ref.height:
        raise ValueError(funcName +
                         ': \"ref\" must be of the same size as \"src\"!')

    bits = src.format.bits_per_sample
    src_16 = core.fmtc.bitdepth(
        src, bits=16, planes=planes) if bits < 16 else src
    src_8 = core.fmtc.bitdepth(
        src, bits=8, dmode=1, planes=[0]) if bits != 8 else src
    ref_16 = core.fmtc.bitdepth(
        ref, bits=16,
        planes=planes) if ref.format.bits_per_sample < 16 else ref

    # Main debanding
    chroma_flag = (thrc != thr or radiusc != radius
                   or elastc != elast) and 0 in planes and (1 in planes
                                                            or 2 in planes)
    ...
    if chroma_flag:
        if 0 in planes2:
            planes2.remove(0)
        flt_c = GF3_smooth(src_16, ref_16, smode, radiusc, thrc, elastc,
                           planes2)
        flt = core.std.ShufflePlanes([flt_y, flt_c],
                                     list(range(src.format.num_planes)),
    ...
    mexpr = 'x {tl} - {th} {tl} - / 255 *'.format(
        tl=td_lo - 0.0001, th=td_hi + 0.0001)
    ...
    if mask > 0:
        ...
        res_16 = core.std.MaskedMerge(
            flt, src_16, dmask, planes=planes, first_plane=True)
    else:
        res_16 = flt

    # Dithering
    result = res_16 if lsb else core.fmtc.bitdepth(
        res_16,
        bits=bits,
        planes=planes,
        dmode=mode,
...
def GF3_smooth(src_16, ref_16, smode, radius, thr, elast, planes):
    ...
    if smode == 0:
        return GF3_smoothgrad_multistage(src_16, ref_16, radius, thr, elast,
                                         planes)
    elif smode == 1:
        return GF3_dfttest(src_16, ref_16, radius, thr, elast, planes)
    elif smode == 2:
        return GF3_bilateral_multistage(src_16, ref_16, radius, thr, elast,
                                        planes)
    elif smode == 3:
...
def GF3_smoothgrad_multistage(src, ref, radius, thr, elast, planes):
    ...
    last = src
    last = SmoothGrad(
        last, radius=r2, thr=thr, elast=elast, ref=ref,
        planes=planes) if r2 >= 1 else last
    last = SmoothGrad(
        last, radius=r3, thr=thr * 0.7, elast=ela_2, ref=ref,
        planes=planes) if r3 >= 1 else last
    last = SmoothGrad(
        last, radius=r4, thr=thr * 0.46, elast=ela_3, ref=ref,
        planes=planes) if r4 >= 1 else last
    return last
...
def GF3_smoothgrad_multistage_3(src, radius, thr, elast, planes):
    ...
    last = mvf.LimitFilter(
        last, src, thr=thr * 0.6, elast=elast, ref=ref, planes=planes)
    return last
...
def GF3_dfttest(src, ref, radius, thr, elast, planes):
    ...
    last = core.dfttest.DFTTest(
        src,
        sigma=hrad * thr * thr * 32,
        sbsize=hrad * 4,
        sosize=hrad * 3,
        tbsize=1,
        planes=planes)
...
def GF3_bilateral_multistage(src, ref, radius, thr, elast, planes):
    ...
    last = core.bilateral.Bilateral(
        src,
        ref=ref,
        sigmaS=radius / 2,
        sigmaR=thr / 255,
        planes=planes,
        algorithm=0)  # The use of "thr" may be wrong
...
def Build_gf3_range_mask(src, radius=1):
    ...
    if radius > 1:
        ma = haf.mt_expand_multi(
            last, mode='ellipse', planes=[0], sw=radius, sh=radius)
        mi = haf.mt_inpand_multi(
            last, mode='ellipse', planes=[0], sw=radius, sh=radius)
        ...
        exp = "x y -"
        exp2 = "x {thY1} < {black} x ? {thY2} > {white} x ?".format(
            thY1=0, thY2=255, black=black, white=white)
...
def AnimeMask(input,
    ...
    if mode not in [-1, 1]:
        raise ValueError(funcName +
                         ': \'mode\' have not a correct value! [-1 or 1]')
    ...
    mask1 = core.std.Convolution(
        input, [0, 0, 0, 0, 2, -1, 0, -1, 0], saturate=True).fmtc.resample(
            sx=shift, sy=shift, **fmtc_args, **resample_args)
    mask2 = core.std.Convolution(
        input, [0, -1, 0, -1, 2, 0, 0, 0, 0], saturate=True).fmtc.resample(
            sx=-shift, sy=-shift, **fmtc_args, **resample_args)
    mask3 = core.std.Convolution(
        input, [0, -1, 0, 0, 2, -1, 0, 0, 0], saturate=True).fmtc.resample(
            sx=shift, sy=-shift, **fmtc_args, **resample_args)
    mask4 = core.std.Convolution(
        input, [0, 0, 0, -1, 2, 0, 0, -1, 0], saturate=True).fmtc.resample(
            sx=-shift, sy=shift, **fmtc_args, **resample_args)
    ...
    if bits != mask.format.bits_per_sample:
        mask = core.fmtc.bitdepth(
            mask, bits=bits, fulls=True, fulld=True, dmode=1)
...
def AnimeMask2(input, r=1.2, expr=None, mode=1):
    ...
    smooth = core.fmtc.resample(
        input, haf.m4(w / r), haf.m4(h / r), kernel='bicubic').fmtc.resample(
            w, h, kernel='bicubic', a1=1, a2=0)
    smoother = core.fmtc.resample(
        input, haf.m4(w / r), haf.m4(h / r), kernel='bicubic').fmtc.resample(
            w, h, kernel='bicubic', a1=1.5, a2=-0.25)
...
def PolygonExInpand(input,
    ...
    if shape not in list(range(3)):
        raise ValueError(funcName +
                         ': \'shape\' have not a correct value! [0, 1 or 2]')

    if mixmode not in list(range(3)):
        raise ValueError(funcName +
                         ': \'mixmode\' have not a correct value! [0, 1 or 2]')
    ...
    while shift > 0:
        ...
        dia = [
            math.sqrt(step / 2),
            math.sqrt(step / 2) * (1 << input.format.subsampling_h)
        ]
        inv_dia = [
            -math.sqrt(step / 2),
            -math.sqrt(step / 2) * (1 << input.format.subsampling_h)
        ...
        if shape == 0 or shape == 2:
            mask2 = core.fmtc.resample(
                mask5, sx=0, sy=ortho, **fmtc_args, **resample_args)
            mask4 = core.fmtc.resample(
                mask5, sx=ortho, sy=0, **fmtc_args, **resample_args)
            mask6 = core.fmtc.resample(
                mask5, sx=inv_ortho, sy=0, **fmtc_args, **resample_args)
            mask8 = core.fmtc.resample(
                mask5, sx=0, sy=inv_ortho, **fmtc_args, **resample_args)

        if shape == 1 or shape == 2:
            mask1 = core.fmtc.resample(
                mask5, sx=dia, sy=dia, **fmtc_args, **resample_args)
            mask3 = core.fmtc.resample(
                mask5, sx=inv_dia, sy=dia, **fmtc_args, **resample_args)
            mask7 = core.fmtc.resample(
                mask5, sx=dia, sy=inv_dia, **fmtc_args, **resample_args)
            mask9 = core.fmtc.resample(
                mask5, sx=inv_dia, sy=inv_dia, **fmtc_args, **resample_args)
        ...
        if noncentral:
            ...
            if shape == 0 or shape == 1:
                ...
                mask5 = core.std.Expr([mask2, mask4, mask6, mask8]
                                      if shape == 0 else
                                      [mask1, mask3, mask7, mask9], [expr])
            else:  # shape == 2
                expr = expr_list[mixmode + 3] + ' {amp} *'.format(amp=amp)
                mask5 = core.std.Expr(
                    [mask1, mask2, mask3, mask4, mask6, mask7, mask8,
                     mask9], [expr])
            ...
            if (shape == 0) or (shape == 1):
                expr = expr_list[mixmode] + ' {amp} *'.format(amp=amp)
                mask5 = core.std.Expr([mask2, mask4, mask5, mask6, mask8]
                                      if shape == 0 else
                                      [mask1, mask3, mask5, mask7,
                                       mask9], [expr])
            else:  # shape == 2
                expr = expr_list[mixmode + 3] + ' {amp} *'.format(amp=amp)
                mask5 = core.std.Expr([
...
def ediaa(a):
    ...
    last = core.fmtc.resample(
        last,
        a.width,
        a.height, [-0.5, -0.5 * (1 << a.format.subsampling_w)],
        [-0.5, -0.5 * (1 << a.format.subsampling_h)],
        kernel='spline36')
...
def maa(input):
    ...
    mask = core.std.Convolution(
        input, [0, -1, 0, -1, 0, 1, 0, 1, 0], divisor=2,
        saturate=False).std.Binarize(scale(7, bits) + 1)
    ...
    if input_src is None:
        ...
        return core.std.ShufflePlanes(
            [last, input_src],
            planes=list(range(input_src.format.num_planes)),
...
def SharpAAMcmod(orig,
    ...
    m = core.std.Expr([
        core.std.Convolution(
            orig, [5, 10, 5, 0, 0, 0, -5, -10, -5], divisor=4, saturate=False),
        core.std.Convolution(
            orig, [5, 0, -5, 10, 0, -10, 5, 0, -5], divisor=4, saturate=False)
    ], [
        'x y max {neutral8} / 0.86 pow {peak8} *'.format(
            neutral8=scale(128, bits), peak8=scale(255, bits))
    ])
    ...
    if aatype == 'sangnom':
        ...
        raise ValueError(
            funcName +
            ': valid values of \"aatype\" are \"sangnom\", \"eedi2\" and \"nnedi3\"!'
        )
    ...
    if sharp == 0 and smooth == 0:
        ...
        postsh = haf.LSFmod(
            aa, strength=sharp, overshoot=1, soft=smooth, edgemode=1)
    ...
    if stabilize:
        ...
        fv3 = core.mv.Analyse(
            origsuper, isb=False, delta=3, overlap=aaov,
            blksize=aablk) if tradius == 3 else None
        fv2 = core.mv.Analyse(
            origsuper, isb=False, delta=2, overlap=aaov,
            blksize=aablk) if tradius >= 2 else None
        fv1 = core.mv.Analyse(
            origsuper, isb=False, delta=1, overlap=aaov,
            blksize=aablk) if tradius >= 1 else None
        bv1 = core.mv.Analyse(
            origsuper, isb=True, delta=1, overlap=aaov,
            blksize=aablk) if tradius >= 1 else None
        bv2 = core.mv.Analyse(
            origsuper, isb=True, delta=2, overlap=aaov,
            blksize=aablk) if tradius >= 2 else None
        bv3 = core.mv.Analyse(
            origsuper, isb=True, delta=3, overlap=aaov,
            blksize=aablk) if tradius == 3 else None
        ...
        if tradius == 1:
            ...
            raise ValueError(funcName +
                             ': valid values of \"tradius\" are 1, 2 and 3!')

        reduc = 0.4
        sDD = core.std.Expr(
            [sD, sDD], [
                'x {neutral} - abs y {neutral} - abs < x y ?'.format(
                    neutral=scale(128, bits))
            ]).std.Merge(sDD, 1.0 - reduc)
    ...
    if orig_src is None:
        ...
        return core.std.ShufflePlanes(
            [last, orig_src],
            planes=list(range(orig_src.format.num_planes)),
...
def TEdge(input, min=0, max=65535, planes=None, rshift=0):
    ...
    gx = core.std.Convolution(
        input, [4, -25, 0, 25, -4], planes=planes, saturate=False, mode='h')
    gy = core.std.Convolution(
        input, [-4, 25, 0, -25, 4], planes=planes, saturate=False, mode='v')

    calcexpr = 'x x * y y * + {rshift} / sqrt'.format(rshift=rshift)
    expr = '{calc} {max} > {peak} {calc} {min} < {floor} {calc} ? ?'.format(
        calc=calcexpr, max=max, peak=peak, min=min, floor=floor)
    return core.std.Expr([gx, gy], [(expr if i in planes else '')
...
def Sort(input, order=1, planes=None, mode='max'):
    ...
    if mode not in ['max', 'min']:
        raise ValueError(funcName +
                         ': valid values of \"mode\" are \"max\" and \"min\"!')
    ...
    if order == 1:
        ...
        sort = core.rgvs.Repair(
            core.std.Maximum(input, planes=planes), input,
            [(order if i in planes else 0)
             for i in range(input.format.num_planes)])
    elif order == 5:
        sort = core.std.Median(input, planes=planes)
    elif order in range(6, 9):
        sort = core.rgvs.Repair(
            core.std.Minimum(input, planes=planes), input,
            [((10 - order) if i in planes else 0)
             for i in range(input.format.num_planes)])
    else:  # order == 9
...
def Soothe_mod(input, source, keep=24, radius=1, scenechange=32,
               use_misc=True):
    """Modified Soothe().
    ...
    if input.format.id != source.format.id:
        raise ValueError(
            funcName + ': \"source\" must be of the same format as \"input\"!')
    if input.width != source.width or input.height != source.height:
        raise ValueError(funcName +
                         ': \"source\" must be of the same size as \"input\"!')
    ...
    if use_misc:
        if not isinstance(radius, int) or (not (1 <= radius <= 12)):
            raise ValueError(funcName +
                             ': \'radius\' have not a correct value! [1 ~ 12]')
    else:
        if not isinstance(radius, int) or (not (1 <= radius <= 7)):
            raise ValueError(funcName +
                             ': \'radius\' have not a correct value! [1 ~ 7]')
    ...
    if source_src is None:
        ...
        return core.std.ShufflePlanes(
            [last, source_src],
            planes=list(range(source_src.format.num_planes)),
    ...
    if scenechange:
        if 'SCDetect' in dir(haf):
            ...
            raise AttributeError(
                'module \"havsfunc\" has no attribute \"SCDetect\"!')

    return core.misc.AverageFrames(
        input, [1 for i in range(2 * radius + 1)], scenechange=scenechange)


def FixTelecinedFades(input,
                      mode=0,
                      threshold=[0.0],
                      color=[0.0],
                      full=None,
    ...
    for i in mode:
        if i not in [0, 1, 2]:
            raise ValueError(funcName +
                             ': valid values of \"mode\" are 0, 1 or 2!')
    ...
    def GetExpr(scale, color, threshold):
        if color != 0:
            flt = 'x {color} - {scale} * {color} +'.format(
                scale=scale, color=color)
        else:
            flt = 'x {scale} *'.format(scale=scale)
        return flt if threshold == 0 else '{flt} x - abs {threshold} > {flt} x ?'.format(
            flt=flt, threshold=threshold)
        ...
        if topAvg != bottomAvg:
            if mode == 0:
                ...
                topField = core.std.Expr([topField], [
                    GetExpr(
                        scale=meanAvg / topAvg,
                        threshold=threshold,
                        color=color)
                ])
                bottomField = core.std.Expr([bottomField], [
                    GetExpr(
                        scale=meanAvg / bottomAvg,
                        threshold=threshold,
                        color=color)
                ])
            elif mode == 1:
    ...
    if not full and not isFloat:
        input = core.fmtc.bitdepth(
            input, fulls=False, fulld=True, planes=planes)
    ...
    for i in range(input.format.num_planes):
        if i in planes:
            ...
            bottomFieldPlanes[i] = mvf.GetPlane(bottomField,
                                                i).std.PlaneStats()
            adjustedPlanes[i] = core.std.FrameEval(
                inputPlane,
                functools.partial(
                    Adjust,
                    clip=inputPlane,
    ...
    adjusted = core.std.ShufflePlanes(
        [(adjustedPlanes[i] if i in planes else input_src)
         for i in range(input.format.num_planes)],
        [(0 if i in planes else i)
         for i in range(input.format.num_planes)], input.format.color_family)
    if not full and not isFloat:
        adjusted = core.fmtc.bitdepth(
            adjusted, fulls=True, fulld=False, planes=planes)
        adjusted = core.std.ShufflePlanes(
            [(adjusted if i in planes else input_src)
             for i in range(input.format.num_planes)],
            list(range(input.format.num_planes)), input.format.color_family)
    return adjusted


def TCannyHelper(input,
                 t_h=8.0,
                 t_l=1.0,
                 plane=0,
                 returnAll=False,
    ...
    strongEdge = core.tcanny.TCanny(
        input, t_h=t_h + 1e-4, t_l=t_h, mode=0, **canny_args)
    weakEdge = core.tcanny.TCanny(
        input, t_h=t_l + 1e-4, t_l=t_l, mode=0, **canny_args)

    expr = "x y and {peak} y {neutral} 0 ? ?".format(
        peak=1.0 if isFloat else (1 << bits) - 1,
        neutral=0.5 if isFloat else 1 << (bits - 1))
    view = core.std.Expr([strongEdge, weakEdge], [expr])

    if returnAll:
        tcannyOutput = core.tcanny.TCanny(
            input, t_h=t_h, t_l=t_l, mode=0, **canny_args)
...
def MergeChroma(clip1, clip2, weight=1.0):
    ...
    if weight >= 1.0:
        ...
        output = core.std.ShufflePlanes([clip1, output_u, output_v], [0, 0, 0],
                                        vs.YUV)
...
def firniture(clip,
    ...
    impulseCoefficents = dict(
        binomial5=[
            8, 0, -589, 0, 11203, 0, -93355, 0, 606836, 1048576, 606836, 0,
            -93355, 0, 11203, 0, -589, 0, 8
        ],
        binomial7=[
            146, 0, -20294, 0, 744006, 0, -11528384, 0, 94148472, 0,
            -487836876, 0, 2551884458, 4294967296, 2551884458, 0, -487836876,
            0, 94148472, 0, -11528384, 0, 744006, 0, -20294, 0, 146
        ],
        maxflat5=[
            -259, 1524, -487, -12192, 17356, 42672, -105427, -85344, 559764,
            1048576, 559764, -85344, -105427, 42672, 17356, -12192, -487, 1524,
            -259
        ],
        maxflat8=[
            2, -26, 166, -573, 912, 412, 1524, -589, -12192, 17356, 42672,
            -105427, -85344, 606836, 1048576, 606836, -85344, -105427, 42672,
            17356, -12192, -589, 1524, 412, 912, -573, 166, -26, 2
        ],
        noalias4=[-1, 2, 4, -6, -17, 7, 59, 96, 59, 7, -17, -6, 4, 2, -1],
        noaliasnoring4=[
            -1, 8, 40, -114, -512, 360, 3245, 5664, 3245, 360, -512, -114, 40,
            8, -1
        ])
    ...
    clip = core.fmtc.resample(
        ...
        height,
        kernel='impulse',
        impulse=impulseCoefficents[kernel],
        kovrspl=2,
        taps=taps,
        **resample_args)
    ...
    if core.std.get_functions().__contains__('BoxBlur'):
        if radius == 2:
            return core.std.Convolution(
                input, [1] * (radius * 2 - 1) * (radius * 2 - 1),
                planes=planes)
        else:
            return core.std.BoxBlur(
                input, hradius=radius - 1, vradius=radius - 1, planes=planes)
    else:
        if radius == 2 or radius == 3:
            ...
            return core.std.Convolution(
                input, [1] * (radius * 2 - 1), planes=planes,
                mode='v').std.Convolution(
                    [1] * (radius * 2 - 1), planes=planes, mode='h')


def SmoothGrad(input,
               radius=9,
               thr=0.25,
               ref=None,
               elast=3.0,
    ...
    return mvf.LimitFilter(
        smooth, input, ref, thr, elast, planes=planes, **limit_filter_args)
...
def DeFilter(input, fun, iter=10, planes=None, **fun_args):
    ...
    # iteration
    for i in range(iter):
        flt = core.std.Expr([flt, input, fun(flt, **fun_args)],
                            [(calc_expr if i in planes else '')
                             for i in range(input.format.num_planes)])
...
def ColorBarsHD(clip=None, width=1288, height=720):
    ...
    blkclip_args = dict(
        format=vs.YUV444P8, length=1, fpsnum=30000, fpsden=1001)

    pattern1_colors = dict(
        Gray40=[104, 128, 128],
        White75=[180, 128, 128],
        Yellow=[168, 44, 136],
        Cyan=[145, 147, 44],
        ...
        Red=[51, 109, 212],
        Blue=[28, 212, 120])
    Gray40 = core.std.BlankClip(
        clip, d, p1, color=pattern1_colors['Gray40'], **blkclip_args)
    White75 = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['White75'], **blkclip_args)
    Yellow = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['Yellow'], **blkclip_args)
    Cyan = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['Cyan'], **blkclip_args)
    Green = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['Green'], **blkclip_args)
    Magenta = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['Magenta'], **blkclip_args)
    Red = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['Red'], **blkclip_args)
    Blue = core.std.BlankClip(
        clip, c, p1, color=pattern1_colors['Blue'], **blkclip_args)
    pattern1 = core.std.StackHorizontal(
        [Gray40, White75, Yellow, Cyan, Green, Magenta, Red, Blue, Gray40])

    pattern2_colors = dict(
        Cyan100=[188, 154, 16],
        plusI=[16, 98, 161],
        White75=[180, 128, 128],
        Blue100=[32, 240, 118])
    Cyan100 = core.std.BlankClip(
        clip, d, p23, color=pattern2_colors['Cyan100'], **blkclip_args)
    plusI = core.std.BlankClip(
        clip, c, p23, color=pattern2_colors['plusI'], **blkclip_args)
    White75 = core.std.BlankClip(
        clip, c * 6, p23, color=pattern2_colors['White75'], **blkclip_args)
    Blue100 = core.std.BlankClip(
        clip, d, p23, color=pattern2_colors['Blue100'], **blkclip_args)
    ...
    Yellow100 = core.std.BlankClip(
        clip, d, p23, color=pattern3_colors['Yellow100'], **blkclip_args)
    Y_Ramp_tmp = core.std.BlankClip(
        clip, c * 7, 1, color=[0, 128, 128], **blkclip_args)
    Y_Ramp = lutspa(
        Y_Ramp_tmp,
        mode='absolute',
        y_expr='220 x * {c} 7 * / 16 +'.format(c=c),
        chroma='copy')
    Y_Ramp = core.resize.Point(Y_Ramp, c * 7, p23)
    Red100 = core.std.BlankClip(
        clip, d, p23, color=pattern3_colors['Red100'], **blkclip_args)
    pattern3 = core.std.StackHorizontal([Yellow100, Y_Ramp, Red100])

    pattern4_colors = dict(
        Gray15=[49, 128, 128],
        Black0=[16, 128, 128],
        White100=[235, 128, 128],
        Black_neg2=[12, 128, 128],
        Black_pos2=[20, 128, 128],
        Black_pos4=[25, 128, 128])
    Gray15 = core.std.BlankClip(
        clip, d, p4, color=pattern4_colors['Gray15'], **blkclip_args)
    Black0_1 = core.std.BlankClip(
        clip,
        round(c * 3 / 2),
        p4,
        color=pattern4_colors['Black0'],
        **blkclip_args)
    White100 = core.std.BlankClip(
        clip, c * 2, p4, color=pattern4_colors['White100'], **blkclip_args)
    Black0_2 = core.std.BlankClip(
        clip,
        round(c * 5 / 6),
        p4,
        color=pattern4_colors['Black0'],
        **blkclip_args)
    Black_neg2 = core.std.BlankClip(
        clip,
        round(c / 3),
        p4,
        color=pattern4_colors['Black_neg2'],
        **blkclip_args)
    Black0_3 = core.std.BlankClip(
        clip,
        round(c / 3),
        p4,
        color=pattern4_colors['Black0'],
        **blkclip_args)
    Black_pos2 = core.std.BlankClip(
        clip,
        round(c / 3),
        p4,
        color=pattern4_colors['Black_pos2'],
        **blkclip_args)
    Black0_4 = Black0_3
    Black_pos4 = core.std.BlankClip(
        clip,
        round(c / 3),
        p4,
        color=pattern4_colors['Black_pos4'],
        **blkclip_args)
    Black0_5 = core.std.BlankClip(
        clip, c, p4, color=pattern4_colors['Black0'], **blkclip_args)
    pattern4 = core.std.StackHorizontal([
        Gray15, Black0_1, White100, Black0_2, Black_neg2, Black0_3, Black_pos2,
        Black0_4, Black_pos4, Black0_5, Gray15
    ])
...
def SeeSaw(clp,
    ...
    if not isinstance(clp, vs.VideoNode) or clp.format.color_family not in [
            vs.GRAY, vs.YUV, vs.YCOCG
    ]:
    ...
    if denoised is None:
        dnexpr = 'x {NRL} + y < x {NRL} + x {NRL} - y > x {NRL} - y ? ?'.format(
            NRL=NRL)
        denoised = core.std.Expr([clp, core.std.Median(clp, [0])], [dnexpr]
                                 if isGray else [dnexpr, ''])
    else:
        ...
        if denoised.format.id != clp.format.id:
            raise TypeError(funcName +
                            ': \"denoised\" the same format as \"clp\"!')
        if denoised.width != clp.width or denoised.height != clp.height:
            raise ValueError(
                funcName +
                ': \"denoised\" must be of the same size as \"clp\"!')
    ...
    tameexpr = 'x {NRLL} + y < x {NRL2} + x {NRLL} - y > x {NRL2} - x {BIAS1} * y {BIAS2} * + 100 / ? ?'.format(
        NRLL=NRLL, NRL2=NRL2, BIAS1=bias, BIAS2=100 - bias)
    tame = core.std.Expr([clp, denoised], [tameexpr])

    head = SeeSaw_sharpen2(tame, Sstr, Spower, Szp, SdampLo, SdampHi, 4)

    if ssx == 1 and ssy == 1:
        last = core.rgvs.Repair(
            SeeSaw_sharpen2(tame, Sstr, Spower, Szp, SdampLo, SdampHi, Smode),
            head, [1])
    else:
        last = core.rgvs.Repair(
            SeeSaw_sharpen2(
                tame.fmtc.resample(xss, yss, kernel='lanczos').fmtc.bitdepth(
                    bits=bits),
                Sstr,
                Spower,
                Szp,
                ...
                SdampHi,
                Smode),
            head.fmtc.resample(xss, yss, kernel='bicubic', a1=-0.2,
                               a2=0.6).fmtc.bitdepth(bits=bits),
            [1]).fmtc.resample(
                ox, oy, kernel='lanczos').fmtc.bitdepth(bits=bits)
    ...
    if NRlimit == 0:
        ...
        NRdiff = core.std.MakeDiff(clp, denoised)
        last = core.std.Expr([clp, NRdiff], [
            'y {neutral} {NRL} + > x {NRL} - y {neutral} {NRL} - < x {NRL} + x y {neutral} - - ? ?'.
            format(neutral=neutral, NRL=NRL)
        ])
    ...
    return last if isGray else core.std.ShufflePlanes(
        [last, clp_src],
        list(range(clp_src.format.num_planes)), clp_src.format.color_family)
...
def SeeSaw_sharpen2(clp, strength, power, zp, lodmp, hidmp, rgmode):
    ...
    def get_lut1(x):
        if x == neutral:
            ...
            return min(
                max(
                    math.floor(neutral + (tmp1 / zp)**power * zp * (
                        strength * multiple) * (1 if x > neutral else -1) * (
                            tmp2 * (tmp3 + lodmp) / ((tmp2 + lodmp) * tmp3)) *
                               ((1 + (0 if hidmp == 0 else (zp / hidmp)**4)) /
                                (1 + (0 if hidmp == 0 else
                                      (tmp1 / hidmp)**4))) + 0.5), 0), peak)

    method = clp.rgvs.RemoveGrain([rgmode] if isGray else [rgmode, 0])
    sharpdiff = core.std.MakeDiff(clp, method, [0]).std.Lut(
        function=get_lut1, planes=[0])
...
def SeeSaw_SootheSS(sharp, orig, sootheT=25, sootheS=0):
    ...
    if not isinstance(sharp,
                      vs.VideoNode) or sharp.format.color_family not in [
                          vs.GRAY, vs.YUV, vs.YCOCG
                      ]:
    ...
    if orig.width != sharp.width or orig.height != sharp.height:
        raise ValueError(funcName +
                         ': \"orig\" must be of the same size as \"sharp\"!')
    ...
    expr1 = 'x {neutral} < y {neutral} < xor x {neutral} - 100 / {SSPT} * {neutral} + x {neutral} - abs y {neutral} - abs > x {SSPT} * y {i} * + 100 / x ? ?'.format(
        neutral=neutral, SSPT=SSPT, i=100 - SSPT)
    expr2 = 'x {neutral} < y {neutral} < xor x {neutral} - 100 / {ST} * {neutral} + x {neutral} - abs y {neutral} - abs > x {ST} * y {i} * + 100 / x ? ?'.format(
        neutral=neutral, ST=ST, i=100 - ST)

    if sootheS != 0:
        last = core.std.Expr([last, core.std.Convolution(last, [1] * 9)],
                             [expr1])
    if sootheT != 0:
    ...
    return last if isGray else core.std.ShufflePlanes(
        [last, orig_src],
        list(range(orig_src.format.num_planes)), orig_src.format.color_family)
...
def abcxyz(clp, rad=3.0, ss=1.5):
    ...
    x = core.resize.Bicubic(clp, haf.m4(ox / rad),
                            haf.m4(oy / rad)).resize.Bicubic(
                                ox, oy, filter_param_a=1, filter_param_b=0)
    y = core.std.Expr([clp, x], [
        'x {a} + y < x {a} + x {b} - y > x {b} - y ? ? x y - abs * x {c} x y - abs - * + {c} /'.
        format(a=scale(8, bits), b=scale(24, bits), c=scale(32, bits))
    ])

    z1 = core.rgvs.Repair(clp, y, [1])

    if ss != 1:
        maxbig = core.std.Maximum(y).resize.Bicubic(
            haf.m4(ox * ss), haf.m4(oy * ss))
        minbig = core.std.Minimum(y).resize.Bicubic(
            haf.m4(ox * ss), haf.m4(oy * ss))
        z2 = core.resize.Lanczos(clp, haf.m4(ox * ss), haf.m4(oy * ss))
        z2 = core.std.Expr([z2, maxbig, minbig],
                           ['x y min z max']).resize.Lanczos(ox, oy)
        z1 = z2  # for simplicity

    if not isGray:
        z1 = core.std.ShufflePlanes([z1, clp_src],
                                    list(range(clp_src.format.num_planes)),
                                    clp_src.format.color_family)
...
def Sharpen(clip, amountH=1.0, amountV=None, planes=None):
    ...
    if amountH < -1.5849625 or amountH > 1:
        raise ValueError(funcName +
                         ': \'amountH\' have not a correct value! [-1.58 ~ 1]')
    ...
    if amountV is None:
        ...
        if amountV < -1.5849625 or amountV > 1:
            raise ValueError(
                funcName +
                ': \'amountV\' have not a correct value! [-1.58 ~ 1]')
...
def Blur(clip, amountH=1.0, amountV=None, planes=None):
    ...
    if amountH < -1 or amountH > 1.5849625:
        raise ValueError(funcName +
                         ': \'amountH\' have not a correct value! [-1 ~ 1.58]')
    ...
    if amountV is None:
        ...
        if amountV < -1 or amountV > 1.5849625:
            raise ValueError(
                funcName +
                ': \'amountV\' have not a correct value! [-1 ~ 1.58]')

    return Sharpen(clip, -amountH, -amountV, planes)


def BlindDeHalo3(clp,
                 rx=3.0,
                 ry=3.0,
                 strength=125,
                 lodamp=0,
    ...
    TWK_HLIGHT = 'x y - abs {i} < {neutral} {TWK} {neutral} {TWK} - {TWK} {neutral} / * + {TWK0} {TWK} {LD} + / * {neutral} {TWK} - {j} / dup * {neutral} {TWK} - {j} / dup * {HD} + / * {neutral} + ?'.format(
        i=scale(1, bits),
        neutral=neutral,
        TWK=TWK,
        TWK0=TWK0,
    ...
    mm = core.std.Expr(
        [sm.std.Maximum(), sm.std.Minimum()],
        ['x y - 4 *']).std.Maximum().std.Deflate().std.Convolution(
            [1] * 9).std.Inflate().resize.Bicubic(
                oxi, oyi, filter_param_a=1, filter_param_b=0).std.Inflate()
    ...
    clean = core.std.Expr(
        [i, smd], ['x y {neutral} - -'.format(neutral=neutral)])
    ...
    if PPmode != 0:
        ...
        small = core.resize.Bicubic(base,
                                    haf.m4(oxi / math.sqrt(rx * 1.5)),
                                    haf.m4(oyi / math.sqrt(ry * 1.5)))
        ex1 = Blur(small.std.Maximum(), 0.5)
        in1 = Blur(small.std.Minimum(), 0.5)
        hull = core.std.Expr(
            [
                ex1.std.Maximum().rgvs.RemoveGrain(11), ex1, in1,
                in1.std.Minimum().rgvs.RemoveGrain(11)
            ],
            ['x y - {i} - 5 * z a - {i} - 5 * max'.format(i=scale(
                1, bits))]).resize.Bicubic(
                    oxi, oyi, filter_param_a=1, filter_param_b=0)

        if abs(PPmode) == 1:
            postclean = core.std.MaskedMerge(base,
                                             small.resize.Bicubic(
                                                 oxi,
                                                 oyi,
                                                 filter_param_a=1,
                                                 filter_param_b=0), hull)
        elif abs(PPmode) == 2:
            postclean = core.std.MaskedMerge(base,
                                             base.rgvs.RemoveGrain(19), hull)
        elif abs(PPmode) == 3:
    ...
    if not isGray:
        last = core.std.ShufflePlanes([last, clp_src],
                                      list(range(clp_src.format.num_planes)),
                                      clp_src.format.color_family)
...
def dfttestMC(input,
    ...
    if pp is not None:
        ...
        if input.format.id != pp.format.id:
            raise ValueError(
                funcName + ': \"pp\" must be of the same format as \"input\"!')
        if input.width != pp.width or input.height != pp.height:
            raise ValueError(
                funcName +
                ': \"flt_h\" must be of the same size as \"input\"!')
    ...
    pp_super = haf.DitherLumaRebuild(
        pp if pp_enabled else input, s0=1, chroma=chroma).mv.Super(
            pel=pel, chroma=chroma)
    super = haf.DitherLumaRebuild(
        input, s0=1, chroma=chroma).mv.Super(
            pel=pel, chroma=chroma) if pp_enabled else pp_super
    ...
    for i in range(1, mc + 1):
        bvec.append(
            core.mv.Analyse(pp_super, delta=i, isb=True, **analysis_args))
        fvec.append(
            core.mv.Analyse(pp_super, delta=i, isb=False, **analysis_args))
    ...
    if mdg:
        ...
        if mc >= 3:
            degrained = core.mv.Degrain3(input, super, bvec[0], fvec[0],
                                         bvec[1], fvec[1], bvec[2], fvec[2],
                                         **degrain_args)
        elif mc == 2:
            degrained = core.mv.Degrain2(input, super, bvec[0], fvec[0],
                                         bvec[1], fvec[1], **degrain_args)
        elif mc == 1:
            degrained = core.mv.Degrain1(input, super, bvec[0], fvec[0],
                                         **degrain_args)
        else:
            degrained = input
    else:
    ...
    # Motion Compensation.
    degrained_super = haf.DitherLumaRebuild(
        degrained, s0=1, chroma=chroma).mv.Super(
            pel=pel, levels=1, chroma=chroma) if mdg else super
    compensate_args = dict(thsad=thSAD, thscd1=thSCD1, thscd2=thSCD2)
    bclip = []
    fclip = []
    for i in range(1, mc + 1):
        bclip.append(
            core.mv.Compensate(degrained, degrained_super, bvec[i - 1], **
                               compensate_args))
        fclip.append(
            core.mv.Compensate(degrained, degrained_super, fvec[i - 1], **
                               compensate_args))

    # Create compensated clip.
    fclip.reverse()
    interleaved = core.std.Interleave(fclip + [degrained] +
                                      bclip) if mc >= 1 else degrained
    ...
    return core.std.SelectEvery(filtered, mc * 2 + 1,
                                mc) if mc > 1 else filtered
